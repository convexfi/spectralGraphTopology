...
setEPS()
cairo_ps("convergence-bipartite.ps", family = "Times", height = 5,
         width = gr * 3.5, fallback_resolution = 1500, pointsize = 14)
par(mar = c(5, 5, 3, 5))
plot(iter, avg_relerr, type = "l", lty = lty[1], col = colors[1],
     xlab = "Iteration number", ylab = "Relative Error", ylim = c(.125, .5), lwd = 2)
...
plot(iter, avg_fscore, type = "l", lty = lty[2], col = colors[2],
     xlab = "", ylab = "", ylim = c(0.4, .75), lwd = 2, xaxt = "n", yaxt = "n")
...
setEPS()
cairo_ps("objective-trend-bipartite.ps", family = "Times", height = 5,
         width = gr * 3.5, fallback_resolution = 1500, pointsize = 14)
par(mar = c(5, 5, 3, 5))
plot(iter, avg_nll, type = "l", lty = lty[1], col = colors[1],
     xlab = "Iteration number", ylab = "Avg. Negative Loglikelihood", lwd = 2, ylim = c(-105, max(avg_nll)))
#polygon(x = c(iter, rev(iter)), y = c(avg_nll - .5*std_nll, rev(avg_nll + .5*std_nll)),
#        col = alpha(colors[1], alpha = .3), border = NA)
par(new = TRUE)
plot(iter, avg_objfun, type = "l", lty = lty[2], col = colors[2],
     xlab = "", ylab = "", lwd = 2, xaxt = "n", yaxt = "n", ylim = c(-105, max(avg_objfun)))
