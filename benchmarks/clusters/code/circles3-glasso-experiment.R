...
graph <- glasso(crossprod(t(circles3$data)), rho = 1e-1)
# construct the ne1work
A <- diag(diag(graph$wi)) - graph$wi
...
gr = .5 * (1 + sqrt(5))
setEPS()
postscript("../latex/figures/circles3-glasso.ps", family = "Times", height = 5, width = gr * 3.5)
