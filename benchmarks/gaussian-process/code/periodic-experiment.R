...
n <- 64
p <- 10 * n
Strue <- periodic_kernel(1, sqrt(2), 120, n)
Y <- MASS::mvrnorm(p, mu = rep(0, n), Sigma = Strue)
graph <- learn_laplacian_matrix(cov(Y), k=1, w0 = "qp", beta = 4)
...
gr = .5 * (1 + sqrt(5))
setEPS()
postscript("../latex/figures/periodic_graph.ps", family = "Times", height = 5, width = gr * 3.5)
plot(est_net, vertex.label = NA, vertex.size = 3)
dev.off()
setEPS()
postscript("../latex/figures/periodic_laplacian.ps", family = "Times", height = 5, width = gr * 3.5)
corrplot(graph$Lw / max(graph$Lw), is.corr = FALSE, method = "square", addgrid.col = NA, tl.pos = "n", cl.cex = 1.25)
dev.off()
postscript("../latex/figures/periodic_covariance.ps", family = "Times", height = 5, width = gr * 3.5)
corrplot(Strue / max(Strue), is.corr = FALSE, method = "square", addgrid.col = NA, tl.pos = "n", cl.cex = 1.25)
dev.off()
setEPS()
postscript("../latex/figures/periodic_precision.ps", family = "Times", height = 5, width = gr * 3.5)
