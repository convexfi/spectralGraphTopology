...
rel_err_cglA  <- readRDS(file = "rel-err-CGLA.rds")
fscore_cglA   <- readRDS(file = "fscore-CGLA.rds")
...
colors <- c("#0B032D", "#843B62", "#F67E7D", "#FFB997", "#6ABA81")
pch <- c(15, 7, 8, 11, 9)
lty <- c(1, 2, 4, 5, 6)
legend <- c("Naive", "QP", "CGL", "CGL (oracle)", "SGL (proposed)")
...
lines(n_ratios, rel_err_spec, type = "b", lty=lty[5], pch=pch[5], cex=.75, col = colors[5], xaxt = "n")
...
lines(n_ratios, fscore_spec, type = "b", lty=lty[5], pch=pch[5], cex=.75, col = colors[5], xaxt = "n")
axis(side = 1, at = n_ratios, labels = ratios)
legend("bottomright", legend=legend, col=colors, pch=pch, lty=lty, bty="o", bg = "white", box.col = "white")
dev.off()
