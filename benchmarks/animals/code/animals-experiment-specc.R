...
run_animals <- function(k) {
  ...
  spec <- specc(t(Y), centers = k)
  cluster <- as.vector(as.numeric(factor(spec)))
  A <- matrix(0, 33, 33)
  for (i in c(1:32))
    for (j in c((i+1):33))
  ...
  V(net)$cluster <- cluster
  V(net)$color <- colors[cluster]
  E(net)$color <- "white"
  gr = .5 * (1 + sqrt(5))
  setEPS()
  postscript("../latex/figures/animals-spectral-clustering.ps")
