...
for (j in 1:length(ratios)) {
  ...
  for (n in 1:N_realizations) {
    ...
    if (ratios[j] <= 50)
      graph <- learn_laplacian_matrix(S, w0 = w_qp, beta = 1e1, abstol = 0, edge_tol = 0, maxiter = 1000000)
    else
      graph <- learn_laplacian_matrix(S, w0 = w_qp, beta = 1e2, abstol = 0, edge_tol = 0, maxiter = 1000000)
