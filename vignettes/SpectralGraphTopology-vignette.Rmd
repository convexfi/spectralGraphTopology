...
res <- learnGraphTopology(cov(Y), K, beta = 10)
```
...
prial <- c(proposed = spectralGraphTopology::prial(Lw, res$Lw),
           naive = spectralGraphTopology::prial(Lw, Lw_naive))
rel_err
prial
```
...
K <- 2
res <- learnGraphTopology(cov(Y), K, w0 = "naive", beta = 100)
...
plot(c(1:N_iter), rel_err_seq, type = "b", pch=19, cex=.6, col = scales::alpha("black", .5),
