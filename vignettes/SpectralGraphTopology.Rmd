...
In many practical cases, the connections between nodes is a known property. For example,
...
unweighted adjacency matrix of the graph is known a priori and we can use state-of-the-art algorithms
that leverages this fact to estimate the edge weights of the graph. In this package,
we implement the method named "GLE-MM", which was proposed in [@Licheng2019]. The graph estimation is carried
as follows:
...
Y <- MASS::mvrnorm(50 * p, mu = rep(0, p), Sigma = MASS::ginv(Ltrue))
# Compute the sample covariance matrix
S <- cov(Y)
# Estimate a graph from the samples
graph <- learn_laplacian_gle_mm(S = S, A = A, alpha = 1e-5, record_objective = TRUE)
...
plot(c(1:length(graph$objective_function)), graph$objective_function, xlab = "Iteration number",
     ylab = "Objective function value", type = "b", pch=15, lty=1, cex=.75, col = "#6ABA81", xaxt = "n")
