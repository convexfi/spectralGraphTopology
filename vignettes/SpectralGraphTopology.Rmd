...
#W_true <- diag(diag(L_true)) - L_true        # true Adjacency matrix
...
2. a quadratic program estimator given by $\textsf{minimize}_\mathbf{w}\quad\Vert\mathbf{S}^{\dagger} - \mathcal{L}\mathbf{w}\Vert_{F}$, where $\mathbf{S}^{\dagger}$ is the generalized inverse of the sample covariance matrix, denoted as QP;
