...
and efficient estimator. In R, this estimator can be computed simply as `S <- cov(t(Y)); solve(S, diag(nrow(S)))`,
where ``Y`` is the $p \times n$ data matrix, $n$ is the number of samples (or features) and $p$ is the number of nodes.
...
``MASS`` package as ``MASS::ginv(cov(t(Y)))``. In practice, these naive techniques perform very poorly, even when $n$ is just a few orders
...
to estimate graph matrices (Laplacian and Adjancency) from data.  We illustrate the practicality of
...
\begin{equation}
\begin{array}{ll}
&\underset{\boldsymbol{\Theta}}{\textsf{maximize}} & \log \mathrm{g}\det \boldsymbol{\Theta}
- \mathrm{tr}(\mathbf{S}\boldsymbol{\Theta}) - \alpha \Vert \boldsymbol{\Theta}\Vert_{1, \text{off}},\\
&\textsf{subject to} & \boldsymbol{\Theta} \in \mathcal{S}_{\mathcal{L}},
\end{array}
\label{eq:cgl}
\end{equation}
where $\mathrm{g}\det \boldsymbol{\Theta}$ represents the generalized determinant, i.e., the product of the positive eigenvalues of $\boldsymbol{\Theta}$, and $\mathcal{S}_{\mathcal{L}}$ is the set of (combinatorial) Laplacian matrices, which may
be written as
$$ \begin{equation}
...
frameworks such as CVX. An CVXR implementation of the above optimization problem can be coded as
...
learn_laplacian_matrix_cvx <- function(S, alpha = 0) {
  ...
  J <- matrix(1/p, p, p)
  H <- alpha * (2 * diag(p) - p * J)
  K <- S + alpha * H
  Theta <- CVXR::Variable(p, p)
  objective <- CVXR::matrix_trace(Theta %*% (K + J)) - CVXR::log_det(Theta + J)
...
We set up a simple experiments to compare the performance of the four approaches:
(1) directly solving with **CVXR**, (2) **CGL**[@Egilmez2017], (3) **GLE-MM** [@Licheng2019], and (4) **GLE-ADMM** [@Licheng2019].
...
Although estimating a graph Laplacian matrix by solving formulation (\ref{eq:cgl}) has shown an extreme potential for success,
this particular formulation only allows for the estimation of connected graphs.  In many practical situations, however,
