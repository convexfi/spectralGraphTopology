...
would be via the inverse of the sample covariance matrix, which is an assymptotically unbiased
...
\end{array}$$ where $\mathcal{S}_{\mathcal{L}}$ is the set of (combinatorial) Laplacian matrices, which may
be written as
$$ \begin{equation}
...
The assumed that the binary adjacency matrix is known
...
and who also follow A. Another example arises in the field of finance, where stocks in the same industry or sector
