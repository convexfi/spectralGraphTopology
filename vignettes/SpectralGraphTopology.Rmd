...
\end{array}
$$
where $\mathbf{S}$ is the sample covariance matrix and $\alpha$ is a hyperparameter
that controls the amount of sparsity of the solution.  The problem above can
be regarded as a regularized maximum likelihood estimation of the precision matrix of
a multivariate Gaussian distribution.  This estimator has been efficiently implemented
in the R package ``glasso``.

In [@Licheng2019], the authors extended the graphical lasso framework so as to
consider $\boldsymbol{\Theta}$ as a graph Laplacian. Mathematically, the set of Laplacian
...
\boldsymbol{\Theta}_{ij} = \boldsymbol{\Theta}_{ji} < 0, \boldsymbol{\Theta} \succeq \mathbf{0}\right\}
\end{equation}
$$
