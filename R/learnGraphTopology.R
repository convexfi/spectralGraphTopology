...
#' @export
learn_bipartite_graph <- function(S, z = 0, w0 = "naive", alpha = 0., nu = 1e4,
                                  Lips = NULL, maxiter = 1e4, Awtol = 1e-4, ftol = 1e-4) {
  # number of nodes
  n <- ncol(S)
  J <- matrix(1/n, n, n)
  # l1-norm penalty factor
  ...
  return(list(Laplacian = Lw, Adjacency = Aw, obj_fun = fun_seq, loglike = ll_seq, w = w,
...
learn_adjacency_and_laplacian <- function(S, z = 0, k = 1, w0 = "naive", alpha = 0.,
                                          beta = 1e4, rho = 1e-2, fix_beta = FALSE, nu = 1e4,
                                          lb = 0, ub = 1e4, maxiter = 1e4, tol = 1e-4, eig_tol = 1e-9) {
  if (nrow(S) != ncol(S)) {
    A <- build_initial_graph(S, m = m)
    D <- diag(.5 * colSums(A + t(A)))
    L <- D - .5 * (A + t(A))
  ...
  return(list(Laplacian = Lw, Adjacency = Aw, obj_fun = fun_seq, loglike = ll_seq, w = w,
              ...
              beta_seq = beta_seq, convergence = !(i == maxiter)))
}
