...
learn_bipartite_graph <- function(S, z = 0, w0 = "naive", alpha = 0., beta = 1.,
  ...
  return(list(Aw = Aw, Lw = Lw, obj_fun = fun_seq,
              ...
              Lips = Lips, Lips_seq = Lips_seq, convergence = !(i == maxiter),
              beta = beta))
}
...
learn_adjacency_and_laplacian <- function(S, z = 0, k = 1, w0 = "qp", alpha = 0.,
  ...
  Aw0 <- A(w0)
  Lw0 <- L(w0)
  V0 <- joint.V_update(Aw0, n, z)
  psi0 <- joint.psi_update(V0, Aw0)
  U0 <- joint.U_update(Lw0, n, k)
  lambda0 <- joint.lambda_update(lb, ub, beta1, U0, Lw0, n, k)
  # save objective function value at initial guess
  ...
  for (i in c(1:maxiter)) {
    ...
    Lw <- L(w)
    Aw <- A(w)
    U <- joint.U_update(Lw, n, k)
    V <- joint.V_update(Aw, n, z)
    lambda <- joint.lambda_update(lb, ub, beta1, U, Lw, n, k)
  ...
  return(list(Lw = Lw, Aw = Aw, obj_fun = fun_seq, loglike = ll_seq, w = w,
              psi = psi, V = V, elapsed_time = time_seq,
