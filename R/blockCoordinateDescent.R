...
#' Updates the value of V, the matrix whose columns represent the eigen vectors
...
#'
#' @return the updated value of U
adajacency.V_update <- function(Aw, n, z) {
  V <- eigenvectors(Aw)
  return(cbind(V[, 1:(.5*(n - z))], V[, (1 + .5*(n + z)):n]))
}


#' Updates the value of w, the weight vector of the graph Adjacency
#'
#' @param w weight vector of the graph
#' @param Lw Laplacian matrix generated by the weight vector of the graph
#' @param U matrix whose columns represent the eigen vectors of the Laplacian
...
#'
#' @return the updated value of w
adjacency.w_update <- function(w, Aw, V, beta, psi, N, Kmat, J) {
  grad_h <- Astar(A(w) - crossprod(sqrt(psi) * t(V))) + Lstar(Kmat) / beta
  w_update <- w - (Lstar(solve(L(w) + J)) + grad_h) / (2 + Lips)
  w_update[w_update < 0] <- 0
  return(w_update)
}
