...
adjacency.w_update <- function(w, Aw, V, beta, psi, Kmat, J, Lips) {
  grad_h <- 2 * w - Astar(crossprod(sign(psi) * sqrt(abs(psi)) * t(V))) +
            Lstar(Kmat) / beta
...
#'
#' @param lb lower bound on the eigenvalues of the Adjacency matrix
#' @param ub upper bound on the eigenvalues of the Adjacency matrix
#' @param β scalar that controls the strength of the regularization term
...
adjacency.psi_update <- function(lb, ub, V, Aw) {
  c <- diag(t(V) %*% Aw %*% V)
  q <- length(c)
  #q <- .5 * length(c)
  #c <- c[(q/2+1) : q]
  ...
  #constraints <- list(0 < psi[1], psi[1:(q - 1)] < psi[2:q], psi[q] < ub)
  constraints <- list(lb < psi[1], psi[1:(q - 1)] < psi[2:q], psi[q] < ub)
  ...
  result <- solve(prob)
  #psi_vec <- as.vector(result$getValue(psi))
  #return(c(-rev(psi_vec), psi_vec))
